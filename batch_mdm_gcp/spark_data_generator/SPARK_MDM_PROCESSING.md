...
## Data Scaling Strategies: PySpark vs. BigQuery

There are two primary methods for generating large-scale data for MDM processing, each with distinct advantages and use cases.

#### 1. PySpark Generator (`spark_data_generator.py`)

This is the primary, high-fidelity method for creating a realistic, enterprise-scale dataset from scratch.

-   **Purpose**: **Generate Net-New, Unique Data.**
-   **Unique Customers**: Creates **truly unique customer identities** (new names, DOBs, etc.).
-   **MDM Challenge**: Simulates a highly realistic scenario where a large, diverse customer base needs to be resolved. This is the best way to showcase the end-to-end power of the MDM solution.
-   **Best For**: Demos focused on the complexity of identity resolution and the full data lifecycle.
-   **Cost/Speed**: Higher cost and slower, as generating unique data is computationally intensive.

#### 2. BigQuery Duplicator (`bq_data_duplicator.sql`)

This is a fast, cost-effective method for inflating the *volume* of records for an existing dataset.

-   **Purpose**: **Scale Existing Record Volume.**
-   **Unique Customers**: **Does NOT create new unique customers.** It multiplies records for the *existing* customer pool generated by Spark.
-   **MDM Challenge**: Simulates having more historical records or interactions for the *same* set of customers.
-   **Best For**: Demos focused on the raw performance of the matching engine or query capabilities against a massive number of records (e.g., "processing 1 billion records").
-   **Cost/Speed**: Much cheaper and extremely fast, as it's a simple SQL transformation.

#### Summary for Demos

| Demo Focus | Recommended Approach | Why? |
| :--- | :--- | :--- |
| **Realistic Identity Resolution** | PySpark Generator | Shows the ability to resolve millions of *truly different* people. |
| **Performance at Record Scale** | BigQuery Duplicator | Quickly creates billions of records to test matching/query speed. |
| **End-to-End Pipeline** | PySpark Generator | Demonstrates the full, enterprise-grade data generation process. |
| **Rapid Demo Prep** | BigQuery Duplicator | Inflates an existing dataset in minutes for a fraction of the cost. |
